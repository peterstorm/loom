...
**T2.2 — Spec anchors compiled to frozen executable acceptance.** At decompose time or wave start, the existing test-engineer agents translate each task's `spec_anchors` into failing unit tests plus property-based tests (fast-check / jqwik), and the engine records their hashes before any implementer spawns (the Fugue tamper-evidence machinery already implements exactly this hash-freeze pattern). Gate: those exact tests pass, hashes unchanged. Consequences:
...
**I6 — Red-then-green as a machine-checked proof obligation.** The substrate already exists and is unused for this: the evidence ledger records ordered `TestRun` facts per agent epoch (`record-evidence`), and `core/proof-obligations.ts` already evaluates ledger-derived claims. Add an obligation: for tasks with `new_tests_required`, the ledger must show a *failing* TestRun on the task's tests before the first `FileWrite` to implementation files, then a passing one after. TDD enforced by arithmetic on facts already collected — no new trust model, no prose protocol the agent can skip. It forces the agent to establish its own oracle before generating against it. Under T2.2 the red state comes for free: the frozen acceptance tests pre-exist and fail by construction.
...
**I8 — Move the deterministic gate to SubagentStop.** Today `applyUntrustedStopResolution` recomputes proof from evidence when the implementer stops. Strengthen it: at stop, the engine runs the deterministic suite scoped to the task — whole-program typecheck, full-tier lint over `files_modified`, the task's tests, architecture rules — and on any failure **rejects the completion and resumes the same agent** with the failure output. This converts an entire class of wave-gate findings into intra-agent iterations while context is hot. It is the same fail-closed philosophy as the reviewer attempt machinery, applied to implementers — except the rejection reason is a compiler message, so the retry converges instead of repeating a hallucinated format. The wave gate then only ever sees tasks that already pass the mechanical bar; reviewers spend their entire budget on semantics.

**I9 — Best-of-N for flagged tasks: spend generation tokens to save remediation tokens.** Once completion is machine-scored (compile + tests + lint + mutation-on-diff), exploit it: for tasks decompose flags as high-risk (novel module, security-touching, wide blast radius), spawn 2–3 implementers in worktree isolation and let the engine pick the candidate with the best deterministic score; a reviewer breaks ties. Sampling-and-selecting is the standard way to convert stochastic generation into near-deterministic output, and it is only rational *because* the selector is code. The isolation machinery and every scoring input already exist. The economics usually favor it: 2× generation on one hard task is cheaper than one remediation cycle (full roster × N tasks + panel + fix + re-review).
...
| 3 | I8 — SubagentStop deterministic gate with same-agent resume | defect distance: gate → completion | medium |
