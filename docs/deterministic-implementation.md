...
**I8 — Add an engine-owned completion oracle at Task and Wave distance.** Today `applyUntrustedStopResolution` recomputes proof from evidence when the implementer stops. Strengthen it in two scopes: Task settlement runs only checks proven file-local and non-observing under exact engine-issued attempt authority; whole-program typecheck, full-tier lint, integrated tests, and architecture checks run after the Wave is quiescent and their accepted suite digest enters protected Wave completion authority. A semantic failure produces one fresh engine-issued attempt in both harnesses, not a same-process continuation promise Pi cannot satisfy. This converts mechanical defects into deterministic diagnostics without false-failing a Task on a concurrently-writing sibling or letting Claude and Pi implement different retry lifecycles.
