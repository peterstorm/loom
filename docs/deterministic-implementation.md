...
**T0.2 — Engine-executed build/typecheck/test as a gate check (Wave scope shipped).** The proof-obligation system establishes *trusted* Task test evidence via ledger attribution, with documented forgery residuals. At Wave scope, the engine now executes operator-frozen project commands after quiescence and records orthogonal exit/timeout/signal/report facts; evidence the engine produced needs no model trust. Required reports live under a protected report root and are freshness-checked. Task-local execution remains part of the later attempt-authority slice because whole-program checks are unsound while sibling Tasks write concurrently.
...
**I8 — Add an engine-owned completion oracle at Task and Wave distance (Task + Wave scopes implemented).** The Wave half runs whole-program project checks and full-tier lint only after quiescence, persists/replays exact Run Directory results, exposes failures through canonical status, and binds accepted suite/workspace authority into the protected completion commit. Slice 3 adds exact engine-issued Implementation Attempt authority to both harnesses and one shared pure Oracle/application path. Its non-empty Task suite contains only `loom:task-byte-scope`: parser-proven transcript paths are checked against the registered `attempt_artifact_baseline`, while cumulative declared-artifact Proof still compares the first `artifact_baseline`. Foreign transcript paths are semantic failure; baseline/path/read/Git uncertainty is infrastructure-blocked; repository dirty-set movement is invalidation evidence only and never attribution or sibling Proof. Task-local settlement executes **no project subprocesses**—build, test, typecheck, reports, package scripts, and full-tier lint remain Wave-quiescent. Exact receipts make duplicate/late results idempotent and release only matching authority. Slice 3 classifies retry-required/escalation-required but dispatches neither; Slice 4 alone freezes retry context and launches semantic attempt 2 or escalation.
