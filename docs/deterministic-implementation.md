...
**I8 — Add an engine-owned completion oracle at Task and Wave distance (Wave scope shipped).** `applyUntrustedStopResolution` still recomputes Task proof from harness evidence. The shipped Wave half runs whole-program project checks and full-tier lint only after quiescence, persists/replays exact Run Directory results, exposes failures through canonical status, and binds accepted suite/workspace authority into the protected completion commit. The remaining Task half will run only checks proven file-local and non-observing under exact engine-issued attempt authority; semantic retry remains a later slice rather than an unsound same-process continuation promise.
