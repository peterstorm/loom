...
export function parseSubagentStartInput(raw: unknown): ParsedSubagentStartInput {
  if (typeof raw !== "object" || raw === null || Array.isArray(raw)) {
    return Object.freeze({ ok: false, error: "SubagentStart input must be a JSON object" });
  }
  const record = raw as Record<string, unknown>;
  if (typeof record.session_id !== "string") {
    return Object.freeze({ ok: false, error: "SubagentStart missing/invalid session id; session_id must be a string" });
  }
  for (const field of OPTIONAL_STRING_FIELDS) {
    if (record[field] !== undefined && typeof record[field] !== "string") {
      return Object.freeze({ ok: false, error: `SubagentStart ${field} must be a string when present` });
    }
  }
...
export function parseSubagentStartStdin(stdin: string): ParsedSubagentStartInput {
  ...
  try {
    ...
    return Object.freeze({
      ok: false,
      error: `SubagentStart input is malformed JSON: ${error instanceof Error ? error.message : String(error)}`,
    });
  }
  return parseSubagentStartInput(raw);
}
