...
- `probes/emission-qualification/recordings/` — 39 committed files, verbatim wire traffic: `NNN-request.json` (every model request body) and `NNN-response.sse` (every raw streamed response) for 17 request/response pairs, plus `001-wire-params.json` (actual serialized v2 wire tool parameters — byte-identical to the frozen v2 schema bytes) and `003-wire-params.json` (actual serialized v3 wire tool parameters — byte-identical to the frozen v3 schema bytes) and the machine stage report `probe-report.json` (`errors: []`).
...
|---|---|---|---|---|
| reviewer v2 | **yes** (HTTP 200, `recordings/001-*`) | true | **yes** | no — model complied under temptation; the forced direct call also emitted shape-nonconforming arguments (string-typed shape, §2.6) → recorded **inconclusive** |
| reviewer v3 | **yes** (HTTP 200, `recordings/007-*`) | true | **yes** | no — model complied under temptation; the forced direct call also emitted shape-nonconforming arguments (string-typed shape, §2.6) → recorded **inconclusive** |
...
- **Wire-parameter identity:** the machine report records `wireParamsEqualFrozen: true` (deep structural equality of the parsed wire parameters against the frozen bytes) for every acceptance/violation stage. The retained artifacts go further: `recordings/001-wire-params.json` is **byte-identical** to the frozen v2 schema bytes in `fixtures/manifest.json`, and `recordings/003-wire-params.json` is likewise **byte-identical** to the frozen v3 schema bytes. The strictifier is a no-op on this route — the zod-derived frozen bytes are already strict-compatible (`additionalProperties: false` + complete `required` everywhere, including across v2's root `oneOf`); pi forwarded the frozen bytes unchanged with `strict: true`.
...
- **The machine stage report contains known bookkeeping artifacts; the committed README verdicts supersede them.**
  - The v2/v3 **direct-enforcement stage arguments violate the frozen bytes as emitted**: the committed raw bodies (`recordings/006-response.sse`, `011-response.sse`) and the machine report's `directEnforcement`/acceptance `rawArgs` carry `schemaVersion` as a string (`'2'`/`'3'`) and `findings`/`priorAssessments` as JSON-encoded strings — shapes the frozen bytes reject (§2.7 observed the validator refusing exactly this class live). The committed violation detector checked only for a `probe_violation` key and therefore recorded `violationEmitted: false`, so the machine report labels v2/v3 "CONSTRAINED" from a check that could not see the shape violation. The route-level classification (§2.5, **unconstrained emission**) rests on the decisive judge/refutation forced violations (`recordings/013`, `015`, `017`) and is unaffected; but the v2/v3 direct stage proves neither enforcement nor model conformance and stays **inconclusive** in both directions.
